            ...
            <div class="flex items-center gap-4">
                @can('update', $product)
                    <div class="flex items-center gap-2">
                        <a href="{{ route('admin.products.edit', $product->slug) }}" class="text-xs font-bold uppercase tracking-widest text-indigo-600 hover:text-indigo-800 transition">{{ __('products.edit_product') }}</a>
                        <span class="text-slate-200">|</span>
                        <form action="{{ route('admin.products.destroy', $product->slug) }}" method="POST" onsubmit="return confirm('Delete this product?');">
                            @csrf @method('DELETE')
