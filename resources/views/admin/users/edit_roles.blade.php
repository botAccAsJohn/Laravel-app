...
@section('title', 'Edit Roles — ' . $targetUser->name)
...
<div class="max-w-4xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            ...
            <h1 class="text-3xl font-black text-gray-900">{{ $targetUser->name }}</h1>
            <p class="text-sm text-gray-500 mt-1">{{ $targetUser->email }}</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.users.update-roles', $targetUser) }}" id="role-form">
        @csrf
        @method('PATCH')
            ...
            <div class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
                ...
                <div class="divide-y divide-gray-50">
                    ...
                    @php
                        ...
                        $isLocked = $role->name === 'customer' && $isAssigned;
                    @endphp
                    <label for="role_{{ $role->id }}"
                           class="flex items-start gap-4 px-6 py-5 {{ $isLocked ? 'opacity-75 cursor-not-allowed' : 'cursor-pointer hover:bg-indigo-50/30' }} transition-colors group">
                        @if($isLocked)
                            {{-- Hidden input ensures the value is still submitted even though the checkbox is disabled --}}
                            <input type="hidden" name="roles[]" value="{{ $role->id }}">
                            <input type="checkbox" checked disabled
                                   class="mt-1 h-4 w-4 rounded border-gray-300 text-gray-400 cursor-not-allowed">
                        @else
                            <input type="checkbox"
                                   id="role_{{ $role->id }}"
                                   name="roles[]"
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 mb-1">
                                <span class="font-bold {{ $isLocked ? 'text-gray-500' : 'text-gray-900 group-hover:text-indigo-700' }} transition-colors">
                                    {{ $role->display_name }}
                                </span>
                                <code class="text-[10px] px-1.5 py-0.5 bg-gray-100 text-gray-500 rounded font-mono">{{ $role->name }}</code>
                                @if($isLocked)
                                <span class="text-[10px] font-black uppercase tracking-widest text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">Base Role · Locked</span>
                                @elseif($isAssigned)
    ...
    <div class="mt-10 bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="p-6">
            @php
                $rolesCollection = $targetUser->roles ?? collect();
